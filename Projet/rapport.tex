...
In our deployment, renewal is handled by a scheduled script executed via
\texttt{cron}, the Linux task scheduler. The script checks the remaining
validity of each certificate, generates a new key and CSR if renewal is due,
submits the CSR to the Intermediate CA, verifies the resulting certificate,
and performs a graceful reload of the web server---all without manual
intervention. This approach mirrors the automation used by widely deployed
systems such as Let's Encrypt's \texttt{certbot}, which renews certificates
automatically when fewer than 30 days of validity remain.

\begin{alertbox}
\footnotesize\textbf{Operational note:} in mTLS deployments, \emph{both}
the server and client certificates must be renewed before expiry. An expired
client certificate causes the same handshake failure as an invalid one.
Automated renewal must therefore cover the full certificate inventory, not
