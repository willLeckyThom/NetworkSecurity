...
\definecolor{coralRed}{RGB}{216,90,48}
\definecolor{amberOrng}{RGB}{186,117,23}
\definecolor{tealGreen}{RGB}{29,158,117}
\definecolor{purpleVio}{RGB}{127,119,221}
\definecolor{softGreen}{RGB}{99,153,34}
...
\definecolor{softGreenLight}{RGB}{234,243,222}
%────────────────────────────────────────────────────────────────────
...
% ── Companion Implementation Guide: cross-reference helpers ──────────────────
% The runnable code (OpenSSL commands, Nginx config, renewal scripts) lives in
% the companion Markdown "Implementation Guide". To turn the references below
% into clickable hyperlinks, replace the empty argument of \IGbase with the URL
% where the guide is hosted (e.g. a GitHub/GitLab "blob" link). Edit the line
% in place; if you leave it empty, the references still read correctly as plain
% text (just without a hyperlink). The step anchors follow GitHub's heading
% convention (lower-case, spaces replaced by hyphens, punctuation removed).
\makeatletter
\newcommand{\IGbase}{https://github.com/willLeckyThom/NetworkSecurity/blob/main/Projet/certificate_chain.md}
\newif\ifIGhref
\ifx\IGbase\@empty\IGhreffalse\else\IGhreftrue\fi
\newcommand{\IGdoc}[1]{\ifIGhref\href{\IGbase}{#1}\else#1\fi}% link to the guide (top)
\newcommand{\IGstep}[2]{\ifIGhref\href{\IGbase\##1}{#2}\else#2\fi}% link to a step anchor
\makeatother
...
\begin{tikzpicture}[
    ...
    eecert/.style ={cert, fill=tealLight,  draw=tealGreen,  text=tealGreen!80!black},
    intcert/.style={cert, fill=amberLight, draw=amberOrng,  text=amberOrng!80!black},
    rootcert/.style={cert, fill=coralLight, draw=coralRed,  text=coralRed!85!black},
    checks/.style={
        rectangle, rounded corners=3pt, draw=purpleVio, line width=0.4pt,
        fill=purpleLight, text=purpleVio!85!black,
        minimum width=32mm, align=left, inner sep=3pt, font=\scriptsize
    },
    sigarr/.style={->, thick, draw=darkgray},
    arrlbl/.style={font=\scriptsize, text=darkgray, midway, fill=white, inner sep=1pt},
    leader/.style={dashed, draw=purpleVio!70, line width=0.3pt}
]
 
% Trust anchor envelope (drawn first, behind the root box)
\node[draw=coralRed!60, dashed, line width=0.5pt, rounded corners=4pt,
      minimum width=58mm, minimum height=20mm,
      label={[font=\scriptsize, text=coralRed!85!black]above:Trusted by client OS / browser}]
      (anchor) at (0,4) {};
 
% Root CA
\node[rootcert] (root) at (0,4)
    {\textbf{Root CA (trust anchor)}\\[-1pt]\scriptsize Self-signed, in trust store};
 
% Intermediate CA
\node[intcert] (interm) at (0,1.6)
    {\textbf{Intermediate CA}\\[-1pt]\scriptsize \texttt{CA:TRUE}, \texttt{pathlen:0}};
 
% End-entity certificate
\node[eecert] (leaf) at (0,-0.8)
    {\textbf{End-entity certificate}\\[-1pt]\scriptsize \texttt{www.cybersec.ulb.ac.be}};
 
% Arrows (leaf -> interm -> root)
\draw[sigarr] (leaf) -- node[arrlbl]{issued by} (interm);
\draw[sigarr] (interm) -- node[arrlbl]{issued by} (root);
 
% Checks panels (right side)
\node[checks, right=8mm of leaf, anchor=west] (cleaf)
    {\textbf{Leaf checks}\\
     \textbullet\ Signature\\
     \textbullet\ Validity dates\\
     \textbullet\ Hostname (SAN)\\
     \textbullet\ EKU = \texttt{serverAuth}\\
     \textbullet\ Not revoked (CRL)};
 
\node[checks, right=8mm of interm, anchor=west] (cint)
    {\textbf{Intermediate checks}\\
     \textbullet\ Signature\\
     \textbullet\ Validity dates\\
     \textbullet\ \texttt{CA:TRUE}\\
     \textbullet\ \texttt{pathLenConstraint}\\
     \textbullet\ Not revoked};
 
\node[checks, right=8mm of root, anchor=west] (croot)
    {\textbf{Root checks}\\
     \textbullet\ Self-signature\\
     \textbullet\ \texttt{CA:TRUE}\\
     \textbullet\ In trust store\\
     \textbullet\ Validity dates};
 
% Leader lines from cert -> check panel
\draw[leader] (leaf.east)   -- (cleaf.west);
\draw[leader] (interm.east) -- (cint.west);
\draw[leader] (root.east)   -- (croot.west);
 
\end{tikzpicture}%
}
\caption{Certificate chain validation. The verifier starts at the end-entity certificate and walks upward to a trusted root, performing the listed checks at each level. The chain is only as strong as its weakest link: a single failure at any tier invalidates the whole connection.}
\label{fig:chain-validation}
\end{figure}
...
\begin{tikzpicture}[
    ...
    clientbox/.style={block, fill=tealLight, draw=tealGreen, text=tealGreen!80!black,
                      minimum width=30mm},
    crlbox/.style ={block, fill=tealLight,    draw=tealGreen,  text=tealGreen!80!black},
    ocspbox/.style={block, fill=purpleLight,  draw=purpleVio,  text=purpleVio!85!black},
    endpt/.style ={block, fill=lightblue!50,  draw=darkgray,
                   text=darkgray, font=\scriptsize\itshape},
    procon/.style={block, fill=amberLight,    draw=amberOrng,
                   text=amberOrng!80!black, align=left,
                   font=\scriptsize, minimum height=11mm, inner sep=3pt},
    arr/.style={->, thick, draw=darkgray},
    arrlbl/.style={font=\scriptsize, text=darkgray, midway, fill=white, inner sep=1pt}
]
 
% Client at the top center
\node[clientbox] (cli) at (0,4)
    {\textbf{Client}\\[-1pt]\scriptsize Verifying a certificate};
 
% ----- LEFT COLUMN: CRL flow -----
\node[crlbox] (crlhead) at (-3.6, 2.6) {\textbf{CRL approach}};
\node[crlbox, below=4mm of crlhead] (crl1)
    {\textbf{1.} Download full CRL\\[-1pt]\scriptsize \texttt{GET /crl/intermediate.crl}};
\node[crlbox, below=4mm of crl1] (crl2)
    {\textbf{2.} Search serial locally\\[-1pt]\scriptsize against revoked list};
\node[endpt, below=4mm of crl2] (crlsrv)
    {CA CRL endpoint (Nginx)\\
     \scriptsize signed, refreshed every 30~days};
\node[procon, below=3mm of crlsrv, minimum width=38mm] (crlpro)
    {+ offline-friendly after download\\
     $-$ heavy, can reach several MB};
 
% Arrows for CRL flow
\draw[arr] (cli.south) .. controls +(-1,-0.5) and +(0,0.5) .. (crl1.north);
\draw[arr] (crl1) -- (crl2);
\draw[dashed, draw=darkgray!60] (crl2) -- (crlsrv);
 
% ----- RIGHT COLUMN: OCSP flow -----
\node[ocspbox] (ocsphead) at (3.6, 2.6) {\textbf{OCSP approach}};
\node[ocspbox, below=4mm of ocsphead] (ocsp1)
    {\textbf{1.} Query one serial\\[-1pt]\scriptsize POST to OCSP responder};
\node[ocspbox, below=4mm of ocsp1] (ocsp2)
    {\textbf{2.} Signed status reply\\[-1pt]\scriptsize Good | Revoked | Unknown};
\node[endpt, below=4mm of ocsp2] (ocspsrv)
    {CA OCSP responder\\
     \scriptsize can be stapled by the server};
\node[procon, below=3mm of ocspsrv, minimum width=38mm] (ocsppro)
    {+ real-time, lightweight per query\\
     $-$ leaks history, fail-soft on error};
 
% Arrows for OCSP flow
\draw[arr] (cli.south) .. controls +(1,-0.5) and +(0,0.5) .. (ocsp1.north);
\draw[arr] (ocsp1) -- (ocsp2);
\draw[dashed, draw=darkgray!60] (ocsp2) -- (ocspsrv);
 
\end{tikzpicture}%
}
\caption{Two revocation-checking strategies compared. The CRL approach (left, used in our deployment) downloads the CA's signed revocation list and matches the certificate's serial number locally. The OCSP approach (right) queries the CA in real time for a single serial. Both are required by the validation steps of Figure~\ref{fig:chain-validation}, and our renewal pipeline regenerates the CRL weekly to avoid the failure mode described in Section~5.}
\label{fig:crl-vs-ocsp}
\end{figure}
...
The remainder of this section follows a two-tier deployment; the complete,
runnable procedure---every OpenSSL invocation, the Nginx configuration, and
the renewal scripts---is provided in the companion \IGdoc{Implementation
Guide}, to which each subsection below links.
...
\begin{tikzpicture}[
    ...
    rootnode/.style={cabox, fill=coralLight, draw=coralRed, text=coralRed!85!black},
    interm/.style ={cabox, fill=amberLight, draw=amberOrng, text=amberOrng!80!black},
    server/.style ={cabox, fill=tealLight,  draw=tealGreen, text=tealGreen!80!black,
                    minimum width=36mm},
    client/.style ={cabox, fill=purpleLight,draw=purpleVio, text=purpleVio!85!black,
                    minimum width=36mm},
    enduse/.style ={font=\scriptsize\itshape, text=darkgray},
    siglabel/.style={font=\scriptsize, text=darkgray},
    arr/.style={->, thick, draw=darkgray}
]
 
% Trust anchor (dashed envelope around root)
\node[rootnode] (root) at (0,0) {\textbf{Root CA}\\[-1pt]\scriptsize Offline, self-signed, 20 years};
 
\node[draw=coralRed!60, dashed, line width=0.5pt, rounded corners=4pt,
      fit=(root), inner sep=4mm,
      label={[font=\scriptsize, text=coralRed!85!black]above left:Trust anchor (air-gapped)}] (anchor) {};
 
% Intermediate CA below
\node[interm, below=14mm of root] (interm)
    {\textbf{Intermediate CA}\\[-1pt]\scriptsize Online, 10 years, \texttt{pathlen:0}};
 
% End-entity row
\node[server, below=18mm of interm, xshift=-26mm] (server)
    {\textbf{Server certificate}\\[-1pt]\scriptsize EKU: \texttt{serverAuth}};
\node[client, below=18mm of interm, xshift= 26mm] (client)
    {\textbf{Client certificate}\\[-1pt]\scriptsize EKU: \texttt{clientAuth}};
 
% Arrows
\draw[arr] (root) -- node[siglabel, right=1pt]{signs} (interm);
 
% Branching arrow from interm to server and client
\coordinate (branch) at ($(interm.south) + (0,-6mm)$);
\draw[arr, -] (interm) -- (branch);
\draw[arr, -] (branch) -| (server.north);
\draw[arr]   (branch) -| (client.north);
\node[siglabel, right=2pt of branch] {issues};
 
% Final consumers
\node[enduse, below=4mm of server] (serveruse) {Nginx (\texttt{www.cybersec.ulb.ac.be})};
\node[enduse, below=4mm of client] (clientuse) {User / device};
\draw[arr] (server) -- (serveruse);
\draw[arr] (client) -- (clientuse);
 
\end{tikzpicture}%
}
\caption{Two-tier Certificate Authority architecture. The Root CA is kept offline and only signs the Intermediate CA, which in turn issues server and client end-entity certificates used in our mTLS deployment.}
\label{fig:two-tier-ca}
\end{figure}

\subsection{Directory Structure}

A well-organized directory structure is essential for auditability. The
working directory for the root CA should contain the following (the commands
that create this layout are given under \IGstep{directory-structure}{Directory
Structure} in the \IGdoc{Implementation Guide}):
...
The exact OpenSSL commands for these operations are listed in
\IGstep{step-1-configure-the-root-ca}{Steps~1--3 of the Implementation Guide}.
...
to relying parties during TLS handshakes. These operations correspond to
\IGstep{step-4-configure-the-intermediate-ca}{Steps~4--7 of the Implementation
Guide}.
...
are then deployed to the web server (e.g.\ Nginx or Apache). The server- and
client-certificate commands are given in
\IGstep{step-8-issue-a-server-certificate}{Steps~8--9 of the Implementation
Guide}.
...
Authority Information Access (AIA) extension of issued certificates. The CRL
generation, revocation, and distribution commands are provided in
\IGstep{step-10-generate-the-initial-crl}{Steps~10--12 of the Implementation
Guide}.
...
The complete \texttt{openssl.cnf} files for the root and intermediate CAs,
including these sections, are reproduced in
\IGstep{step-1-configure-the-root-ca}{Step~1} and
\IGstep{step-4-configure-the-intermediate-ca}{Step~4} of the
\IGdoc{Implementation Guide}.
...
\begin{figure}[ht]
    \centering
    \includegraphics[scale=0.3]{certificate_image.png}
...
The server-side configuration that enforces this exchange
(\texttt{ssl\_verify\_client on}) and the \texttt{curl} tests that validate it
are given in \IGstep{step-13-configure-nginx-for-mtls}{Steps~13--14 of the
...
\texttt{openssl.cnf} (\IGstep{step-4-configure-the-intermediate-ca}{Step~4}
of the \IGdoc{Implementation Guide}).
\end{infobox}
...
\begin{tikzpicture}[
    ...
    clientbox/.style={party, fill=tealLight, draw=tealGreen, text=tealGreen!80!black},
    serverbox/.style={party, fill=purpleLight, draw=purpleVio, text=purpleVio!85!black},
    msg/.style={->, thick, draw=darkgray},
    msglbl/.style={font=\scriptsize, text=darkgray, midway, fill=white, inner sep=1pt},
    check/.style={
        rectangle, rounded corners=2pt, fill=amberLight, draw=amberOrng,
        line width=0.4pt, text=amberOrng!80!black,
        align=center, inner sep=2pt, font=\scriptsize
    },
    finalbox/.style={
        rectangle, rounded corners=3pt, fill=softGreenLight, draw=softGreen,
        line width=0.4pt, text=softGreen!75!black,
        align=center, inner sep=3pt, minimum width=55mm
    }
]
 
% Header parties
\node[clientbox] (cli) at (0, 0) {\textbf{Client}};
\node[serverbox] (srv) at (7, 0) {\textbf{Server}};
 
% Named coordinates for the message lines (no more inline calc-coords)
\coordinate (cli1) at ($(cli.south)+(0,-0.8)$);
\coordinate (srv1) at ($(srv.south)+(0,-0.8)$);
\coordinate (cli2) at ($(cli.south)+(0,-1.8)$);
\coordinate (srv2) at ($(srv.south)+(0,-1.8)$);
\coordinate (cliv) at ($(cli.south)+(0,-2.6)$);
\coordinate (cli3) at ($(cli.south)+(0,-5.5)$);
\coordinate (srv3) at ($(srv.south)+(0,-5.5)$);
\coordinate (cliend) at ($(cli.south)+(0,-7)$);
\coordinate (srvend) at ($(srv.south)+(0,-7)$);
 
% Lifelines
\draw[dashed, draw=darkgray!50] (cli.south) -- (cliend);
\draw[dashed, draw=darkgray!50] (srv.south) -- (srvend);
 
% Message 1: ClientHello
\draw[msg] (cli1) -- node[msglbl]{\textbf{1.} ClientHello} (srv1);
 
% Message 2: ServerHello + Certificate + CertificateVerify + Finished

\draw[msg] (srv2) -- node[msglbl,align=center]
    {\textbf{2.} ServerHello, EncryptedExtensions,\\ 
     Certificate, CertificateVerify, Finished} (cli2);
 
% Client verification box, positioned relative to cliv coordinate
\node[check, below=3mm of cliv, anchor=north] (verif)
    {Client verifies server flight:\\
     chain, validity, SAN, EKU=\texttt{serverAuth},\\ revocation, CertificateVerify, Finished};
 
% Message 3: Finished
\draw[msg] (cli3) -- node[msglbl]{\textbf{3.} Finished} (srv3);
 
% Application data
\node[finalbox] at ($(cli)!0.5!(srv) + (0,-6.7)$)
    {Encrypted application data\\[-1pt]\scriptsize Symmetric session keys};
 
\end{tikzpicture}%
}
\caption{Standard TLS~1.3 handshake. Only the server presents a certificate; the client remains anonymous. The dashed lines represent the client and server lifelines.}
...
\coordinate (cli2)  at ($(cli.south)+(0,-1.8)$);
\coordinate (srv2)  at ($(srv.south)+(0,-1.8)$);
\coordinate (cliv1) at ($(cli.south)+(0,-2.7)$);
\coordinate (cli3)  at ($(cli.south)+(0,-5.0)$);
\coordinate (srv3)  at ($(srv.south)+(0,-5.0)$);
\coordinate (cli4)  at ($(cli.south)+(0,-5.0)$);
\coordinate (srv4)  at ($(srv.south)+(0,-5.0)$);
\coordinate (srvv2) at ($(srv.south)+(0,-5.8)$);
\coordinate (cliend) at ($(cli.south)+(0,-8.5)$);
\coordinate (srvend) at ($(srv.south)+(0,-8.5)$);
...
% Client verifies server (left side)
\node[check, below=3mm of cliv1, anchor=north, xshift=-1.5mm] (verif1)
    {Client verifies server flight:\\
     chain, SAN, EKU, revocation,\\ 
     CertificateVerify, Finished};
 
% Step 3: client sends its cert
\draw[msg] (cli3) -- node[msglbl, align=center]
    {\textbf{3.} Certificate,\\ CertificateVerify, Finished} (srv3);


 
% Server verifies client (right side)
\node[check, below=3mm of srvv2, anchor=north, xshift=1.5mm] (verif2)
    {Server verifies client flight:\\
     chain, EKU=\texttt{clientAuth}, revocation,\\
     CertificateVerify, Finished};
 
% Application data
\node[finalbox] at ($(cli)!0.5!(srv) + (0,-8)$)
    {Encrypted application data\\[-1pt]\scriptsize Both parties authenticated};
 
\end{tikzpicture}%
}
\caption{Mutual TLS handshake. In addition to the standard TLS exchange, the server issues a \texttt{CertificateRequest}, the client presents its own certificate and a \texttt{CertificateVerify} signature, and the server validates the client certificate against the configured CA chain before deriving session keys.}
...
\begin{tabularx}{\columnwidth}{>{\bfseries\footnotesize}p{2cm}
                                >{\footnotesize}X
                                >{\footnotesize}X}
\toprule
...
\texttt{cron}, the Linux task scheduler
(\IGstep{step-17-create-the-renewal-script}{Step~17} of the
\IGdoc{Implementation Guide}, scheduled in
\IGstep{step-19-schedule-everything-with-cron}{Step~19}). The script is
idempotent: it inspects every certificate in its inventory and triggers a
renewal only when the remaining validity falls below a 30-day threshold,
mirroring the behaviour of Let's Encrypt's \texttt{certbot}. When renewal is
required, the script generates a fresh key pair and a new CSR, submits it to
the Intermediate CA, and verifies the resulting certificate against the chain
...
before its 30-day \texttt{nextUpdate} boundary
(\IGstep{step-18-create-the-crl-refresh-script}{Step~18 of the Implementation
Guide}), since an expired CRL would cause Nginx to reject every connection
regardless of the validity of the end-entity certificates. The two jobs are
kept on independent schedules because they concern different state---the
end-entity certificate inventory on one side, the revocation database on the
other---and decoupling them keeps each script focused on a single
responsibility.
...
the \texttt{-passin file:} option
(\IGstep{step-16-handle-the-encrypted-intermediate-ca-key}{Step~16 of the
Implementation Guide}). This is a deliberate trade-off: a
...
script described in Section~5 and implemented in
\IGstep{step-17-create-the-renewal-script}{Step~17 of the Implementation
Guide}.
...
(\IGstep{step-17-create-the-renewal-script}{Step~17 of the Implementation
Guide}) is designed to prevent, and it is also the philosophy behind tools
such as \texttt{certbot} for Let's Encrypt.
...
The resulting, fully tested procedure is documented end-to-end in the
companion \IGdoc{Implementation Guide}.
