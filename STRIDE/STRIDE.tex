...
To mitigate this attack, a VPN with certificate-based
...
% ------------------------------------------------------------
\subsection{A3 : Passive Traffic Interception / Sniffing (Information Disclosure)}
% ------------------------------------------------------------

For this attack, the attacker passively captures network traffic to
extract sensitive information. The attack exploits flows F1 and F3,
which carry unencrypted communications between the external zone and
the office network, and between the office network and the servers.
In the absence of any VPN or transport-layer encryption, all traffic
traverses the network in plaintext. An attacker positioned anywhere
along the path --- including the Malicious PC on the same external
segment as the Remote Employee --- can use a packet analyser such as
Wireshark to silently read credentials, session tokens, HTTP requests
and responses, or any other sensitive data exchanged across these
flows. Unlike active attacks, sniffing leaves no trace on the target
systems and is therefore particularly difficult to detect. The risk
score is Likelihood 4 and Impact 4, resulting in \textbf{16 (High)}.

To mitigate this attack, a site-to-site VPN with certificate-based
authentication should be deployed to encrypt all traffic between
the Remote Employee and the office network (flow F1). For internal flows between the office network and the servers (flow F3), the firewall deployed in Milestone 1 reduces the attack surface by restricting access to authorised hosts only, limiting the potential for sniffing on that segment.
...
\begin{tabular}{clccc}
  ...
  A3 & Information Disclosure & 4 & 4 & 16 --- High     \\
...
direct control over a server. \textbf{A2} and \textbf{A3} (scores 16,
High) are both easy to execute from outside the network and directly
target the confidentiality of the enterprise: A2 by impersonating a
legitimate user, A3 by passively reading all unencrypted traffic.
Both are directly mitigated by the VPN. \textbf{A5}
(score 15, High) requires a precondition --- having already compromised
a server --- which lowers its likelihood. \textbf{A4} (score 12, Elevated) is easy to
...
\begin{tabularx}{\textwidth}{cXXc}
  ...
  A3 & Passive sniffing / plaintext traffic (I)
     & VPN encryption + Firewall restrictions & 16 \\
...
and the \textbf{VPN}. A3 is equally serious and is mitigated by
the VPN for external flows and by the firewall for internal flows. A5, while serious, will be
addressed as a direct effect of the firewall deployment. A4 will be
